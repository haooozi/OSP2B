...
class WaymoSiameseDataset(Dataset):
    ...
    def getitem(self, index, sample_idx=None, return_raw=False):
        ...
        sample_PC, sample_label, sample_reg, maxi, mini, maxi_xyz, mini_xyz = utils.cropAndCenterPC_label(
        ...
        ret = {
            ...
            'maxi': torch.from_numpy(maxi).float(),
            'mini': torch.from_numpy(mini).float(),
            'maxi_xyz': torch.from_numpy(maxi_xyz).float(),
            'mini_xyz': torch.from_numpy(mini_xyz).float()
        }
...
class WaymoTrackingDataset(WaymoDataset):
    ...
    def __init__(self, output_dir,
                 ...
                 training=True,
                 root_path=Path('/media/xxx/waymo'),
                 logger=logging,
                 data_ratio=0.1,
                 ):
...
if __name__ == '__main__':
    ...
    for category, ratio in zip(['Vehicle', 'Pedestrian', 'Cyclist'], [0.1, 0.2, 1.0]):
        ds = WaymoTrackingDataset('/media/xxx/waymo_tracking/train/', training=True, class_names=[category], data_ratio=ratio)
        ds.preprocess()

        ds = WaymoTrackingDataset('/media/xxx/waymo_tracking/test/', training=False, class_names=[category], data_ratio=ratio)
        ds.preprocess()
