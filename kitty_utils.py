...
def getmaskPC_train(PC, maxi, mini):
    """
    ...
    4. pt inside box = 1
    """
    # print(PC.shape) #(b,128,3)
    # print(maxi.shape) #(b,3)
    # print(mini.shape)  # (b,3)

    a, b, c = PC.size()
    maxi = maxi.unsqueeze(1).repeat(1, 128, 1)
    mini = mini.unsqueeze(1).repeat(1, 128, 1)
    # PC = PC.cpu().numpy()
    # maxi = maxi.cpu().numpy()
    # mini = mini.cpu().numpy()
    dist1 = PC-mini #(b,128,3) b,r,d
    dist2 = maxi-PC #(b,128,3) f,l,u

    x_min = torch.minimum(dist1[:, :, 0], dist2[:, :, 0])
    x_max = torch.maximum(dist1[:, :, 0], dist2[:, :, 0])
    y_min = torch.minimum(dist1[:, :, 1], dist2[:, :, 1])
    y_max = torch.maximum(dist1[:, :, 1], dist2[:, :, 1])
    z_min = torch.minimum(dist1[:, :, 2], dist2[:, :, 2])
    z_max = torch.maximum(dist1[:, :, 2], dist2[:, :, 2])
...
def cropAndCenterPC_label(PC, sample_box, gt_box, sample_offsets,
    ...
    """============add============"""
    box_tmp = copy.deepcopy(new_box_gt)
    maxi_xyz = np.max(box_tmp.corners(), 1)
    mini_xyz = np.min(box_tmp.corners(), 1)

    box_tmp.wlh = box_tmp.wlh * 0.5
    maxi = np.max(box_tmp.corners(), 1)
    mini = np.min(box_tmp.corners(), 1)

    return new_PC, new_label, label_reg, maxi, mini, maxi_xyz, mini_xyz
