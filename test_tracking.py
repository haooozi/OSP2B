            ...
            for tracklet_idx, (PCs, BBs, list_of_anno) in enumerate(batch): # tracklet
                ...
                for i, _ in enumerate(PCs):
                    ...
                    if i == 0:
                        ...
                        classification_scores = output_dict['classification_scores'].squeeze().detach().cpu().numpy()

                        offsets = output_dict['offsets']
                        angles_scores = output_dict['angles_scores']
                        estimation_boxs_cpu = torch.cat((offsets, angles_scores),dim=2).squeeze().detach().cpu().numpy()
                        center_scores = angles_scores[:, :, 1].squeeze().sigmoid().detach().cpu().numpy()

                        classification_scores = (classification_scores - classification_scores.min()) \
                                                / (classification_scores.max() - classification_scores.min())

                        final_score = center_scores*classification_scores
                        box_idx = final_score.argmax()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--report_dir', type=str, default='/checkpoints', help='output folder')
    parser.add_argument('--data_dir', type=str, default = '/media/xxx/KITTI',  help='dataset path')
    parser.add_argument('--model', type=str, default = 'car.pth',  help='model name for training resume')
    ...
    parser.add_argument('--penalty-k', default='0.05,1.05,0.05', type=parse_range)

    args = parser.parse_args()

    logging.basicConfig(format='%(asctime)s %(message)s', datefmt='%Y/%m/%d %H:%M:%S', \
                    filename=os.path.join(args.report_dir, datetime.now().strftime('%Y-%m-%d %H-%M-%S.log')), level=logging.INFO)
    logging.info('======================================================')
    ...
    for epoch in range(max_epoch):
        ...
        model_dir = os.path.join(args.save_root_dir, 'netR_{}.pth'.format(epoch + 1))
        if args.model != '':
            netR.load_state_dict(torch.load(model_dir), strict=False)
